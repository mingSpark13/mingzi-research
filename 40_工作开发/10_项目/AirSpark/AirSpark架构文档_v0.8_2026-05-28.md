...
**平台总体架构设计 v0.8**
维护人：花火｜日期：2026-05-28
来源对照：`AirSpark架构文档_v0.7_2026-05-21.md`、`D:\AirSpark` 实际仓库（分支 `feat/data-collection-9.0`，2026-05-28）、P3 方向讨论（`研究支撑/初步讨论结果.md`）
本版变更：P3 方向从 SceneOps（程序化场景生成）调整为 AnnotationOps（语义标注数据工厂）；新增场景自标注系统和轨迹语言标注系统架构；SceneSpec 角色降级为场景元数据治理；更新代码架构快照（新增 Annotation/LLM/SemanticMap 模块规划）；更新开发路线图和执行清单；保留 v0.7 全部定位、竞品、任务体系、评测指标、架构原则内容。
...
## 六、P3 AnnotationOps 语义标注数据工厂（当前阶段）

> **方向调整（2026-05-28）**：P3 从 SceneOps（程序化场景生成）调整为 AnnotationOps（语义标注数据工厂）。PCG 已有成熟商业方案（UnrealLLM、AutoUE、Fab），AirSpark 的护城河在"把任何场景变成可训练的低空任务数据"。

### 6.1 P3 核心目标

把"能采集"升级为"能理解场景、能标注轨迹、能生成可训练数据"。

一句话概括：**AirSpark 的重点不是造世界，而是把已有世界变成"可理解、可记忆、可推理、可训练"的低空任务数据。**
...
| 自研 mini-PCG 引擎 | 接入任何外部场景 + 自动标注 |
| 数据飞轮靠场景多样性 | 数据飞轮靠标注深度和语义丰富度 |

原因：
- 程序化场景生成已有大量商业方案全力推进，自研性价比极低
- 场景生成是供给侧，标注与任务构造才是差异化价值
- 同一个 School 地图，通过深度语义标注 + 多样化语言指令 + 推理监督，数据价值可以提升 10×
...
P3 的核心不再是 SceneSpec 驱动 PCG，而是以下五张图：
...
| 阶段 | 内容 | 关键产出 |
|---|---|---|
| P3.0 | 收口采集系统 | ObserveSelector >50%、max_retries、Semantic Inventory 路由 |
| P3.1 | LLMClient + SemanticEnricher | School 地图对象语义补全（canonical_name/affordances/aliases） |
| P3.2 | RegionBuilder + RegionSubsystem | 区域图 v1（走廊/教室/操场自动检测 + LLM 命名） |
| P3.3 | SceneScanner + Stencil 分割 | 完整场景自标注 pipeline（扫描→检测→融合→标注） |
| P3.4 | TrajectoryAnnotator | 轨迹推理数据（实时几何 + 离线 LLM 语言标注） |
| P3.5 | Dataset Export + QA + Failure Mining | 可训练数据集 + 质量验证 + 失败回流 |

### 6.8 P3 交付目标

| 交付物 | 说明 |
|---|---|
| LLMClient 基础设施 | UE 内 OpenAI-compatible HTTP 客户端，支持多模态 |
| SemanticMap v1 | School 地图完整语义地图（ObjectGraph + RegionGraph + RelationGraph） |
| 场景自标注 Pipeline | 一键扫描 → 标注 → 输出 SemanticMap.json |
| TrajectoryAnnotator | 采集时实时标注 + 采集后 LLM 语言标注 |
| TaskTrace 数据集 | 带推理监督的轨迹数据（subgoal/memory/instruction） |
| QA Dashboard | 标注质量可视化，覆盖率分析 |
| Failure Mining | 从失败 episode 自动识别失败模式，驱动补采 |

详细设计见 `D:\AirSpark\docs\design\annotation-ops-architecture.md`。
...
## 十、开发路线图（P3→P4→P5）
### P3：AnnotationOps 语义标注数据工厂（当前）
目标：把场景变成可理解、可记忆、可推理、可训练的低空任务数据。

核心交付：
- LLMClient 基础设施：UE 内 OpenAI-compatible HTTP 客户端
- 场景自标注 Pipeline：SceneScanner + Stencil 分割 + LLM 语义补全 + RegionBuilder
- SemanticMap v1：School 地图完整语义地图（ObjectGraph + RegionGraph + RelationGraph）
- TrajectoryAnnotator：实时几何标注 + 离线 LLM 语言标注
- TaskTrace 数据集：带推理监督的轨迹数据（subgoal/memory/instruction）
- QA Dashboard + Failure Mining：标注质量验证 + 失败回流

P3 前置收口（P3.0）：
- 验证 ObserveSelector 通过率 >50%（已优化候选采样 + clearance 统一）
- 实现 `max_retries_per_episode`
- Semantic Inventory 导出路由（P3.1 前置）
...
## 十一·五、BatchSpec 入口治理（已闭环 / 2026-05-26）
...
### 12.3 SceneSpec 是场景元数据治理（角色降级）
SceneSpec 不再驱动程序化场景生成（PCG 已有成熟商业方案），而是作为"场景元数据"统一描述场景来源、资产版本、区域层级、标注策略。后续接任何外部场景（School、Fab、PCG、3DGS、真实回流）都通过 SceneSpec 接入 AnnotationOps 流水线。

### 12.4 数据飞轮的正确顺序
不要跳步：先语义标注（P3 AnnotationOps）→ 再 Sim2Real 配对（P4）→ 再操作任务（P5）。每一层都依赖上一层的基础设施。数据价值的提升靠标注深度和语义丰富度，不靠场景数量。
...
当前最大问题不是缺想法，而是：
1. **文档一致性**：ROADMAP 与 README/docs 事实源不一致
2. **批采集鲁棒性**：max_retries 未消费，ObserveSelector 通过率低
3. **规划器真实能力**：OccupancyGridPlanner 仍是 stub

P3 的核心任务是收口这些问题，同时建立 SceneSpec 平台化能力，为 P4 Sim2Real 和 P5 操作 Benchmark 打好基础。

**优先级排序：**
1. P2 遗留收口（max_retries + 文档清理）
