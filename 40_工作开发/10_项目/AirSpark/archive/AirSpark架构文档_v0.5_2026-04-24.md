...
# 城市低空多模态数据采集与任务评测平台
**平台总体架构设计 v0.5**
维护人：花火｜日期：2026-04-24
来源对照：`Unreal/airworld/linshi.md` v0.4、`ROADMAP.md`、`README.md`、本地 `Unreal/airworld` 代码结构
本版变更：将计划从"Phase 0/早期 Phase 1"校准为"仿真内核已基本打通，进入程序化场景、数据采集、建图/规划/导航基础设施建设阶段"。明确：高带宽基础设施不应构建在 RPC 数据面上；RPC 保留为调试、远程控制和低频控制面；MuJoCo 后端、采集、建图、规划均应模块化按需启停。
---

## 一、项目定位与差异化
### 1.1 一句话定位
首个同时覆盖「导航→操作」全链路、支持动力学感知评测与 Sim2Real 配对验证的城市低空多模态 benchmark。
### 1.2 竞品分析

| 竞品 | 核心特点 | 我们的差异 |
|---|---|---|
| OpenFly（ICLR 2026） | 4 引擎统一，100K 轨迹，自动生成 | ❌ 纯导航，无操作 |
| OpenUAV（ICLR 2025） | UE4+AirSim，12K 条 6-DoF，三档助手 | ❌ 纯导航，无操作 |
| HUGE-Bench | 3DGS 数字孪生，过程导向评测 | ❌ 纯 VLA 指令，无接触操作 |
| AIR-VLA | 首个空中操作数据集 | 仅实验室桌面，无城市场景 |
...
| 维度 | 我们 | AIR-VLA | DroneVLA | OpenFly | HUGE-Bench |
|---|---|---|---|---|---|
| 导航→操作全链路 | ✅ 6 类操作 + 导航衔接 | ✅ 实验室 | ✅ 1-DoF | ❌ | ❌ |
| 接触操作多样性 | ✅ 6 类 | 部分 | 仅抓取 | ❌ | ❌ |
| 城市实际场景 | ✅ | 实验室 | 实验室 | 城市导航 | 数字孪生 |
| 动力学感知 | ✅ 含负载变化 | 部分 | ❌ | ❌ | 仅碰撞 |
| Sim2Real 配对 | ✅ | ❌ | 无配对 | 3DGS | ❌ |
| 力/力矩数据 | ✅ | ❌ | ❌ | ❌ | ❌ |
| 自动化采集 | ✅ 6 类脚本 | ❌ | ❌ | ✅ 导航 | ❌ |
...
核心卖点：
1. 首个导航→操作全链路空中 benchmark，6 类接触操作 + 城市实际场景
2. 自动化操作数据采集流水线，支持大规模 episode 生成
3. 动力学感知评测，含负载变化、接触力、飞行稳定性
4. Sim2Real 配对数据，可量化仿真-真实 gap
### 1.5 平台服务目标
平台同时服务于以下五类核心能力：
...
### 8.3 自动化数据采集 Pipeline
每个 episode 的自动采集流程：
1. **初始化** → 随机生成 UAV 起点 + 目标物位姿 + 环境条件
2. **导航阶段** → 自动路径规划飞到目标物附近（复用 Phase 1）
3. **接近阶段** → 精细对齐目标物（视觉伺服 / 力反馈引导）
4. **操作阶段** → 执行具体操作（抓取/接触/推拉/清洁等）
5. **撤离阶段** → 安全撤离 + 返回/前往下一目标
6. **记录** → 全程传感器 + 力/力矩 + 末端状态 + 成功/失败标注
自动化变体生成维度：
- 目标物位姿随机化：高度 / 朝向 / 遮挡程度
- 环境条件随机化：风速 / 光照 / 动态障碍
- 操作难度随机化：目标物重量 / 表面摩擦 / 形状复杂度
- 指令变体：同一任务用不同自然语言描述
Phase 2 采集目标量：
...
**v0.5 完整变更（2026-04-24）**：从"Phase 0 文档冻结"校准为"Phase 1.5 — Infrastructure Pivot"；明确 RPC 仅作控制面、数据面走 UE 内部 ObservationBus；确立 MuJoCo 为可选后端；确立模块化按需启停原则；新增 Phase A–G 完整路线图；新增 EpisodeRuntime、ObservationBus、Collector、Mapping、Navigation 模块设计；同步保留项目定位、竞品分析、论文路线、六类操作任务体系、评测指标体系；删除 Phase 0/早期 Phase 1 的过时表述；更新对外定位。
