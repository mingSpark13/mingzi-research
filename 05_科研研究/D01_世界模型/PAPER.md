...
> 方向：D01 世界模型 | 目标会议：ICRA 2027 | 状态：🟡 成形
> 最后更新：2026-06-20
...
6. **C6: Test-Time Multi-Path Latent Reasoning Support**: We introduce a test-time scaling axis for the supervisor based on parallel latent reasoning paths with reward-guided aggregation, and demonstrate that this design preserves the original low-latency action interface while exposing a richer candidate pool for deployment-time ranking. We explicitly separate **proposal-side multi-path gain** from **consume-honest packet-contract gain** to prevent the latter from being overstated.
...
### 2.6 Memory-Augmented Action-Conditioned World Models for Persistent Manipulation

A distinct but increasingly relevant family of world models attacks a problem that D01 has been tracking from the opposite direction: how to make action-conditioned rollouts remain coherent over **long, partially observed** horizons rather than only within the same episode fragment. **Mem-World** [REF: 2606.18960] makes this contribution explicit by attaching a long-horizon memory module on top of a standard action-conditioned world model. The memory module continuously compresses historical observation and action traces and emits a task-aware memory token that participates jointly with the current visual token in future-frame prediction. Mem-World's reported gains in rollout fidelity, video FVD, and downstream policy success over observation-only baselines are accompanied by an explicit cross-episode memory reset / replay protocol, which positions the model as a **persistent task simulator** rather than a single-frame predictor.

For D01, Mem-World sharpens two distinct questions that the current Related Work discussion has only partially separated. First, on the **representation side**, persistent memory tokens can plausibly improve packet ranking because the imagined future is no longer collapsed onto the current frame; this is structurally similar to MemoryVLA++ [REF: 2606.09827] but is rooted in action-conditioned video rollouts rather than in retrieval-augmented control. Second, on the **interface side**, the cross-episode reset / replay protocol makes it possible to test whether a packet that is refreshed across an episodic boundary still preserves the same remaining-plan semantics when the downstream controller finally consumes it. This is exactly the delayed-consumption question central to D01, and the Mem-World protocol gives us a concrete experimental handle on it.

However, the D01 interpretation boundary must remain conservative. Mem-World's gains are reported on rollout fidelity and downstream policy success, not on **consume-time packet survival under delayed uptake**. A persistent simulator may produce longer, more coherent imagined futures while still failing to preserve the same clause identity, remaining-plan anchor, or contact-semantic interpretation when those futures are eventually bound and consumed by an aerial-manipulation controller under refresh-induced drift. In particular, the cross-episode memory reset protocol, although useful for training, does not by itself certify that a released packet survives a delayed consumption handoff. We therefore treat Mem-World as evidence for a **persistent task simulator** family that can plausibly support D01's refresh-stability claims, while reserving the consume-honest promotion for cases where the resulting packet also passes the same `W2/B1–B2` stress cells used elsewhere in this paper.

The Mem-World family also connects naturally to D05's data-flywheel side of our broader research arc: a persistent task simulator is precisely the kind of rollout backbone that the data-flywheel direction wants to feed with disturbance-bucketed promotion. D01 should therefore not silently absorb Mem-World's gains as if they were already deployment-time supervisory progress, but should instead explicitly mark which dimension is being credited — representation-side persistence, refresh-side stability, or actually delayed-consume packet survival.

### 2.7 Test-Time Scaling and Latent Multi-Path Reasoning

A separate but increasingly relevant family of evidence concerns **test-time scaling through latent multi-path reasoning** rather than through parameter, data, or depth scaling. Two recent local anchors make this concrete. **MPCoT** [REF: 2606.06245] argues that long-horizon VLA control need not rely on explicit text chain-of-thought, which incurs non-trivial interface delay and breaks the action interface; instead, MPCoT expands multiple parallel candidate reasoning paths in latent space and aggregates them under a reward-guided scorer that fuses expert-action consistency, world-model/VLM progress, and success feedback. The result is reported to maintain the original 8-step action interface and zero text-reasoning tokens while improving long-horizon performance on LIBERO and CALVIN, with explicit depth–width test-time scaling. **WLA-0** [REF: 2606.05979] takes a complementary route by unifying world modeling, language reasoning, and action synthesis inside one autoregressive transformer and exposing world prediction as a switchable test-time compute source, which yields stronger multi-step reasoning when activated. Together, MPCoT and WLA-0 define a new test-time scaling axis that is fundamentally different from scaling parameters, data, or iterative depth: it scales the **breadth and selection quality of latent reasoning trajectories** at consumption time.

For D01, this matters because deployment-time supervision is itself a multi-candidate ranking problem. A released packet is effectively the highest-scoring candidate under the supervisor's reward, and the quality of that selection is bounded by how rich the candidate pool is and how reliable the scorer is. Test-time multi-path reasoning therefore offers a natural way to **enrich the supervisor's candidate pool** without changing the world model backbone itself. At the same time, MPCoT's reward fusion is structurally similar to WEAVER-style [REF: 2606.13672] planner-side reward prediction, and WLA-0's switchable world prediction shares the same refresh-cost concerns as our WEAVER-style and LoopWM-style [REF: 2606.18208] discussions. We therefore expect MPCoT-style gains to be partially explainable by **(i)** richer candidate breadth, **(ii)** stronger reward-guided aggregation, and **(iii)** better proposal-side reasoning depth, while the unresolved deployment-time question remains whether multi-path reasoning improves **consume-honest packet survival** after those proposal-side gains are subtracted.

This consideration is especially important for aerial manipulation, where the VLM forward latency of explicit text chain-of-thought is genuinely incompatible with millisecond-level flight control. A latent multi-path design preserves the same low-latency action interface while still allowing richer reasoning, which makes it a promising candidate for the supervisor's internal candidate generation. However, D01's deployment contract still requires that this latent multi-path support be reported under a separate family rather than credited as a packet-contract gain by default.
...
(40) Memory-augmented action-conditioned world models such as Mem-World [REF: 2606.18960] achieve longer-horizon coherent rollouts by injecting a persistent task simulator with explicit cross-episode memory reset / replay, but their reported gains are measured on rollout fidelity and downstream policy success rather than on **delayed-consumption packet survival under aerial-manipulation drift**. Without an explicit consume-time contract audit, persistent simulation quality is easy to overclaim as deployment-time supervisory progress.
(41) RL-augmented world-action models such as WAM-RL [REF: 2606.17906] close an important representation-action coupling by binding world-model reconstruction quality to action-head updates through a reconstruction reward plus online video SFT, but their gains are demonstrated on same-cycle long-horizon success rather than on **consume-time packet survival after a refresh-induced gap**. Without explicitly de-synchronizing the world-model refresh and the action-head update at evaluation time, it is hard to know whether the same coupling still holds once the downstream controller uptake is delayed.
(42) Persistent simulator and RL-augmented WAM evidence also jointly suggest that D01's audit vocabulary is no longer complete if it only tracks single-step representation gain and single-step action gain: when refresh, reset, and RL-update events are all potentially involved, the consume-time contract must separately account for **reset-coupled support**, **synchronous RL support**, and **cross-episode memory replay honesty** rather than collapsing them into a generic "long-horizon world model" claim.
...
Recent local evidence from MemoryVLA++ [REF: 2606.09827], iMaC [REF: 2606.09813], and WEAVER [REF: 2606.13672] suggests that D01 still lacks one explicit audit layer between packet freshness and downstream consume-time honesty: **did the packet preserve the right temporal context and contact semantics of the same executable episode, or did it merely stay feasible after refresh?** MemoryVLA++ shows that long-horizon manipulation quality depends strongly on whether retrieved episodic history and imagined near-future context are threaded into action generation, rather than being compressed away into a current-frame-only latent. iMaC complements this by showing that action representation itself can preserve motion/contact structure much more faithfully when rewritten into image-like action tokens. WEAVER then adds a third pressure: even if fidelity, consistency, and efficiency improve together, a deployment-time supervisor still needs to say which future trace and which reward-supporting evidence justified the released packet.

We therefore extend the D01 packet contract with a temporal-context and contact-semantics tuple

\[
\tau_t = (e_t^{\text{episode}},\; f_t^{\text{imagined-future}},\; c_t^{\text{contact-sem}},\; b_t^{\text{bind-honest}},\; u_t^{\text{consume-honest}}),
\]

where `episode` denotes the minimal retrieved execution history attached to the packet, `imagined-future` denotes the compact future trace that justified release, `contact-sem` denotes the packet's local motion/contact-intent semantics, and the last two fields preserve the usual downstream honesty split. The key rule is that a packet cannot be credited as delayed-consumption progress merely because it remained rank-positive, identity-stable, or freshly refreshed. If the downstream controller binds a packet whose attached episode context no longer matches the active execution thread, or whose contact semantics have been weakened into a generic actuation suggestion, then the event is frozen below full handoff credit.

Operationally, this gives D01 a sharper subtraction protocol. Gains from faster refresh, stronger semantic ranking, or cleaner object-slot preservation must first be discounted before asking whether the same **episode-local context** and **contact-intent semantics** survived through actual consume. If not, the evidence remains at most **temporal-context support** or **contact-semantics support**. Only when a packet preserves the same executable episode trace and contact-intent meaning through bind and consume may D01 promote the evidence to a stronger **temporal-context-preserving deployment-time supervisor** claim.
...
### 3.37 Controller-Consume Accountability after WEAVER-style Multi-View World Modeling

The latest local re-read of WEAVER [REF: 2606.13672] sharpens a deployment-time distinction that D01 should make explicit. WEAVER is valuable not merely because it improves world-model fidelity, but because it jointly raises **fidelity**, **long-horizon consistency**, and **efficiency**, then demonstrates gains in policy evaluation, policy improvement, and test-time planning on real robotic manipulation. For D01, this is strong support that deployment-facing supervision should not be justified by a single rollout-quality scalar. However, WEAVER still leaves open a stricter handoff question central to our paper: even if a model imagines better futures more quickly, does the released packet remain **controller-consume honest** when the downstream stack eventually binds and uses it under delayed uptake?

We therefore extend the packet audit with a WEAVER-style evidence tuple

\[
\upsilon_t = (c_t^{\text{corr}},\; \ell_t^{\text{long-horizon}},\; \nu_t^{\text{refresh-cost}},\; b_t^{\text{bind-honest}},\; u_t^{\text{consume-honest}}),
\]

where `corr` measures whether imagined trajectories remain correlated with real downstream outcomes, `long-horizon` measures whether the future trace preserves coherent multi-step semantics rather than local visual plausibility, `refresh-cost` records whether the evidence can still be refreshed cheaply enough before controller use, and the last two fields preserve the usual bind/consume accountability split. The key reporting rule is conservative: gains in correlation, consistency, or refresh efficiency are treated as **upstream evidence quality gains** unless a positive residual remains at true downstream consume.

This gives D01 a cleaner interpretation boundary for recent stronger world models. A WEAVER-style architecture may dramatically improve ranking quality, imagined future coherence, and planner-time throughput, yet the delayed-consumption contract still fails if those better futures are not preserved through the same executable clause, temporal context, and contact semantics at controller consume time. Accordingly, if a row improves `corr`, `long-horizon`, and `refresh-cost` but not `consume-honest`, the result freezes as **multi-view evidence support** rather than full deployment-time supervisory progress. Only when the same packet remains bind-honest and consume-honest after those stronger world-model-side gains are subtracted may D01 promote the evidence to a stronger **controller-consume-accountable deployment-time supervisor** claim.

### 3.38 Latent-Depth Support under Shared-Parameter Looped World Models

The most recent local anchor **LoopWM** [REF: 2606.18208] adds a third orthogonal scaling axis for D01 to internalize: **iterative latent depth** with shared parameters. Rather than scaling parameter count or data volume, LoopWM iterates a single transformer block over the environment latent to refine it adaptively, and reports up to 100× parameter-efficiency gains with comparable or better long-horizon consistency. For D01, this is relevant not as a "yet-another-scaling-axe" curiosity, but because deployment-time supervision on aerial platforms is fundamentally a **latent-refresh-cost problem**: a packet is only useful if it can be re-imagined cheaply enough to stay fresh before downstream consume. A looped block that pays a small fixed parameter cost but spends the saved budget on more refinement iterations naturally maps onto our refresh-budget framing.

We therefore introduce a latent-depth audit tuple

\[
\lambda_t = (p_t^{\text{params}},\; i_t^{\text{iters}},\; r_t^{\text{refresh-cost}},\; \ell_t^{\text{long-horizon}},\; u_t^{\text{consume-honest}}),
\]

where `params` records the parameter footprint, `iters` is the realized iteration count (fixed or adaptive), `refresh-cost` measures whether the looped refinement can be paid for inside the deployment-time refresh budget, `long-horizon` measures whether the iterated latent actually improves multi-step consistency, and `consume-honest` is the same D01 contract residual. The reporting rule mirrors the WEAVER-style discipline: any gain that can be explained by cheaper refresh alone must be subtracted before being credited to the packet contract. If looped refinement lowers `refresh-cost` but the packet still fails to preserve `consume-honest`, the gain is frozen as **latent-depth refresh support**, not as a packet-contract gain. If the looped block lowers `params` and `refresh-cost` while `consume-honest` is preserved, the gain is frozen as **refresh-affordable support**. Only when residual `consume-honest` remains positive after subtracting both refresh-affordable and long-horizon-side gains may D01 promote the evidence to **latent-depth preserving packet-contract support**. This matters because adaptive-iteration policies can be tempted to "loop forever" under distribution shift; D01 should therefore also require a hard iteration cap `i_t \le I_{\max}` as part of the same audit, otherwise a deployment-facing looped world model could become an uncontrolled refresh-cost source rather than a controllable supervisor.

### 3.39 3D-Latent + Ego-Motion Disentanglement as a Geometry-Accountable Refresh Prior

The other recent local anchor, **FR3D** [REF: 2606.18250], takes a complementary route: instead of iterating the latent, it lifts world-model state to a **persistent 3D latent space** and disentangles ego-motion from environment dynamics, predicting a stream of 3D latent futures rather than 2D video pixels. Single-monocular input is enough to produce stable 2-second-ahead dynamic 3D reconstruction with markedly better geometric consistency than 2D-feature world models, and the model zero-shot transfers to new scenes via teacher-student distillation from a foundation model. For D01, the value is not the prediction quality per se, but that **ego-motion is treated as the action proxy for an otherwise action-weak video pretraining corpus**, which is exactly the kind of signal the data-flying-wheel side of D05 is hungry for.

We therefore introduce a geometry-disentanglement audit tuple

\[
\gamma_t = (g_t^{\text{3D-geom}},\; e_t^{\text{ego}},\; a_t^{\text{action-proxy}},\; \ell_t^{\text{long-horizon}},\; u_t^{\text{consume-honest}},\; s_t^{\text{sim2real}}),
\]

where `3D-geom` measures whether the predicted latent preserves persistent 3D occupancy / geometry across time, `ego` measures whether the agent-motion component is correctly separated from environment dynamics, `action-proxy` measures whether ego-motion is interpretable as a supervision signal for downstream manipulation (i.e., whether the disentangled ego stream can be reused as a weak action label), `long-horizon` measures multi-step geometric consistency, `consume-honest` is the same D01 contract residual, and `sim2real` records whether the 3D latent survives domain shift without retraining. The reporting rule is again conservative: 3D geometric quality alone is **planner-side geometric support**. If ego-motion separation only helps the proposal side, the gain freezes as **ego-action proxy support**. If the 3D latent helps the same downstream thread preserve `consume-honest` after subtracting proposal-side gains, the row is promoted to **geometry-preserving packet-contract support**. If the 3D latent also survives distribution shift without retraining, that side-axis is reported separately as **sim2real geometric refresh support**, so reviewers can see that the D01 paper is not silently over-attributing geometric backbone strength to packet-contract quality. Most importantly for D05, the FR3D-style action proxy should be tracked as a distinct data-flying-wheel side-input rather than as a substitute for action-labeled demonstrations, so the paper's contract language can later describe a deployment-time supervisor that is fed by a *mixed* stream of explicit action labels, ego-disentangled proxy labels, and pure video pretraining, each with a separately-audited refresh role.


### 3.40 Persistent Task Simulator with Cross-Episode Memory as Refresh-Stable Latent Support

The most recent local D01 sweep adds **Mem-World** [REF: 2606.18960] as a complementary support family that is structurally different from WEAVER [REF: 2606.13672], LoopWM [REF: 2606.18208], and FR3D [REF: 2606.18250]. Whereas those three improve the world model along the dimensions of joint fidelity / consistency / efficiency, latent-depth scaling, and 3D latent geometry respectively, Mem-World attacks the **temporal-coherence** dimension directly: it adds a long-horizon memory module on top of an action-conditioned world model so that the imagined future is no longer collapsed onto the current observation. The memory module continuously compresses historical observation and action traces into a task-aware memory token that participates jointly with the current visual token in future-frame prediction, and the cross-episode reset / replay protocol positions the model as a persistent task simulator rather than a single-frame predictor.

For D01, Mem-World matters not because it improves ranking fidelity per se, but because it gives the deployment-time supervisor a richer **what-future-was-imagined** evidence source at refresh time. A packet released under a persistent task simulator is, in principle, backed by an imagined future that already includes the previously-clipped or currently-occluded parts of the trajectory. This makes the resulting packet more honest under the **temporal-context preservation** family introduced in §3.36 and gives Mem-World a structural reason to be placed alongside MemoryVLA++ [REF: 2606.09827] in the D01 support taxonomy. However, Mem-World's contribution lives one step removed from the consume-honest handoff question, because a persistent simulator still does not, by itself, expose whether the imagined future remains clause-consistent, contact-semantically intact, and address-stable when the downstream controller finally binds and consumes it under delayed uptake.

We therefore introduce a **persistent task simulator (PTS)** family and a corresponding audit tuple

\[
\mu_t = (m_t^{	ext{mem-token}},\; p_t^{	ext{long-horizon}},\; c_t^{	ext{consume-honest}},\; a_t^{	ext{address-stable}},\; r_t^{	ext{reset-protocol}},\; s_t^{	ext{sim2real}}),
\]

where `mem-token` measures the quality of the persistent memory token itself (compression fidelity, replay stability, drift across refresh), `long-horizon` measures whether the token-conditioned future preserves coherent multi-step semantics, `consume-honest` is the same D01 contract residual, `address-stable` measures whether the memory token still points at the same object / waypoint / clause after cross-episode reset, `reset-protocol` records whether the cross-episode reset / replay was triggered explicitly (training side) versus implicitly (deployment side), and `sim2real` records whether the persistent simulator survives distribution shift without retraining.

The reporting rule mirrors the WEAVER / LoopWM / FR3D discipline introduced earlier in this section. If Mem-World-style gains mostly improve `mem-token` and `long-horizon` but `consume-honest` collapses under delayed uptake, the result is frozen as **persistent simulator representation support**. If `consume-honest` improves only when the cross-episode `reset-protocol` is replayed in lock-step with refresh, the result is frozen as **reset-coupled support**. Only when `consume-honest` and `address-stable` are jointly positive in the `W2/B1` and `W2/B2` consume-time stress cells, and the reset protocol is not silently forced at consume time, may the row be promoted to **persistent-task-simulator-subtracted packet-contract residual**.

This matters because Mem-World is the kind of paper a reviewer would naturally cite as evidence that "long-horizon world modeling is solved, therefore D01's supervisor is no longer needed." D01's intended answer is the opposite. A persistent simulator helps explain *why a released packet may carry richer temporal evidence*, but it does not, on its own, certify *why that richer evidence remains bind-honest and consume-honest after refresh, after episodic reset, and after a delayed downstream controller uptake*. By placing Mem-World into the same subtraction discipline as WEAVER, LoopWM, FR3D, COMET, and RepWAM, D01 keeps persistent-simulation quality and packet-contract quality explicitly separate, and protects the central narrative from a category of upstream-representation gains that, however real, are not the same thing as delayed-consume packet honesty. Operationally, any future Mem-World-style ablation in D01's evaluation bundle must therefore log at minimum `(mem-token, long-horizon, consume-honest, address-stable, reset-protocol, sim2real)` per row, run at least the same three stress cells used elsewhere (`W1/B1, W2/B1, W2/B2`), and explicitly report whether the persistent simulator's `address-stable` channel survives a refresh-then-reset-then-consume cycle. Without these, a persistent-simulator result must freeze at the strongest `mem-token / long-horizon`-justified support family and is not allowed to inherit any packet-contract language, even if its rollout-fidelity number is the highest in the table.

### 3.41 RL-Augmented World-Action Model as Action-Honest Refresh Prior

The latest local D01 sweep also absorbs **WAM-RL** [REF: 2606.17906], which adds a distinct new axis to the support taxonomy by introducing a **reconstruction reward** into the World-Action Model training loop. Unlike the representation-side improvements of RepWAM [REF: 2606.13674] or the multi-view evidence of WEAVER [REF: 2606.13672], WAM-RL binds the world-model reconstruction quality directly to the action-head update through an RL signal, while also performing Online Video SFT in which the world model generates self-supervised video rollouts that continue to fine-tune the action head from its own imagined futures. On CALVIN, this pushes long-horizon success beyond the imitation-learning WAM baseline, and it formalizes a coupling between world-model-side evidence and action-side behavior that D01 has been tracking as a missing link.

For D01, the key contribution of WAM-RL is not that it improves the imagined future per se — that role is already filled by WEAVER, FR3D, and LoopWM in our taxonomy — but that it produces an action-head update rule which is, by construction, sensitive to whether the world model can still faithfully reconstruct the scene after a refresh. This makes WAM-RL the first local anchor that exposes a **closed loop between representation-side refresh and action-side honesty**: if the world model is refreshed in a way that degrades reconstruction quality, the reconstruction reward immediately penalizes the action head, so the released packet is less likely to silently degrade at the consume layer even when the same imagined future might have looked plausible under a pure IL loss.

However, this also sharpens a D01 interpretation boundary. WAM-RL's reported gains are measured on long-horizon CALVIN success, not on **delayed-consumption packet survival under aerial-manipulation drift**. The reconstruction reward is computed online during training and at the moment of action selection, not at the moment of downstream consumption after a refresh-induced gap. It is therefore entirely possible for WAM-RL to improve same-cycle action quality while leaving the consume-honest question exactly as open as it was for purely IL-trained WAMs.

We therefore introduce a **RL-augmented WAM (RWAM)** family and a corresponding audit tuple

\[

ho_t = (q_t^{	ext{recon-reward}},\; o_t^{	ext{online-sft}},\; h_t^{	ext{action-honest}},\; c_t^{	ext{consume-honest}},\; d_t^{	ext{drift}},\; s_t^{	ext{sim2real}}),
\]

where `recon-reward` measures how strongly the action head is shaped by world-model reconstruction quality during training and at action time, `online-sft` records whether the action head is updated from self-generated rollouts (and how often), `action-honest` measures whether the released action remains consistent with the same clause / waypoint anchor / contact intent after refresh, `consume-honest` is the same D01 contract residual, `drift` measures whether the same calibration still holds under domain shift between training reconstruction targets and deployment-time observation, and `sim2real` records whether the RL-augmented WAM survives direct transfer to a real aerial-manipulation platform.

The reporting rule is again conservative. If WAM-RL-style gains mostly improve `recon-reward` and `online-sft` but `action-honest` and `consume-honest` both collapse under delayed uptake, the result is frozen as **RL-shape support**. If `action-honest` improves only under matched training / deployment scenes, the result is frozen as **same-domain RL support**. If `consume-honest` improves only when the world-model refresh happens to coincide with the action-head update step, the result is frozen as **synchronous RL support**. Only when `action-honest`, `consume-honest`, and `drift` are jointly positive across `W1/B1`, `W2/B1`, and `W2/B2`, with the reconstruction reward remaining active even after a refresh-induced gap, may the row be promoted to **RL-augmented packet-contract residual**.

This matters because WAM-RL is the closest local evidence we have to a model in which representation-side and action-side quality are explicitly coupled. D01 should not, however, treat that coupling as automatic proof of consume-time packet survival. The very mechanism that makes WAM-RL strong — namely, that the action head is updated online from world-model rollouts — also means that any deployment-time drift between training reconstruction targets and aerial-manipulation observation may destabilize the calibration in ways that the reported CALVIN numbers do not expose. By placing WAM-RL into the same subtraction discipline as Mem-World, WEAVER, LoopWM, FR3D, COMET, and RepWAM, D01 keeps RL-augmented action honesty and packet-contract quality explicitly separate. Operationally, any future WAM-RL-style experiment in D01's evaluation bundle must therefore log at minimum `(recon-reward, online-sft, action-honest, consume-honest, drift, sim2real)` per row, run at least the same three stress cells used elsewhere, and explicitly report whether the RL augmentation survives a refresh-then-action-then-consume cycle in which the world-model side and the action-head side are intentionally de-synchronized. Without these, an RL-augmented WAM result must freeze at the strongest `recon-reward / online-sft`-justified support family and is not allowed to inherit any packet-contract language.
...
Recent local additions from MemoryVLA++ [REF: 2606.09827], iMaC [REF: 2606.09813], and WEAVER [REF: 2606.13672] suggest that D01 should make one more accountability split explicit. MemoryVLA++ shows that long-horizon embodied execution benefits from preserving retrieved episodic context and imagined future state inside the control pipeline, rather than repeatedly acting from a shallow current-frame summary. iMaC shows that action representation itself can be lifted into motion/contact image tokens, preserving contact-rich actuation semantics that are often lost in low-dimensional action vectors. WEAVER adds a third pressure from the deployment side: long-horizon usefulness only becomes persuasive when fidelity, consistency, and efficiency are jointly strong enough to support policy evaluation, policy improvement, and test-time planning on the same world-model backbone. For D01, these three signals point to a deployment-time question that current packet-contract language has only implicitly covered: even if object identity, clause identity, and thread identity survive refresh, can the packet still preserve the **same temporal episode context** and **same contact-intent semantics** when the downstream stack finally consumes it, and does that preservation survive under a refresh budget realistic enough for deployment?
...
This addition gives D01 a cleaner way to absorb recent long-horizon world-model evidence without overclaiming. MemoryVLA++ should currently be treated as strong support for **temporal-context preservation**, not automatic proof of deployment-time supervisory honesty. iMaC should currently be treated as strong support for **contact-semantic packetization**, not automatic proof of consume-honest handoff. WEAVER should currently be treated as strong support for **joint fidelity-consistency-efficiency budgeting**, not automatic proof that faster or more coherent rollouts preserve packet meaning through delayed consume. The paper's main claim therefore remains conservative: memory-rich, contact-aware, and efficiency-strong world models are valuable precisely because they may make delayed-consumption packets more auditable, but they still need explicit packet-level accounting before being credited as deployment-time supervision gains.
...
### 4.3.5 Object-Centric Planner Support versus Packet-Contract Residual

Recent local evidence from COMET [REF: 2606.14418] adds a useful but bounded support family for D01: **object-centric causal planning**. COMET improves early planning quality by binding actions to object slots and running search over slot-level latent futures, which is directly relevant for candidate ranking, object disambiguation, and counterfactual branch quality. However, these gains still live primarily on the proposal side unless they survive the same delayed downstream bind-and-consume boundary that defines D01's deployment-time supervisor problem.

We therefore introduce an object-centric subtraction tuple

\[
\Omega_t^{obj} = (OAR,\; CPS,\; IAS,\; BHR^{obj},\; CHR^{obj}),
\]

where `OAR` denotes object-attention relevance, `CPS` denotes causal-planning support from slot-level search, `IAS` denotes identity-address stability across emit/refresh/bind/consume, `BHR^{obj}` denotes bind-honest residual after subtracting object-centric planning support, and `CHR^{obj}` denotes consume-honest residual after the same subtraction. The reporting rule is conservative: if COMET-style gains mostly improve proposal ranking or early object disambiguation, they are frozen as **object-centric proposal support**. If slot identity remains stable through bind but the packet still loses clause or referent honesty at consume time, the gain is frozen as **address-stable bind support**. Only when positive `CHR^{obj}` remains after subtracting proposal-side object-centric support may we upgrade the evidence to **object-aware packet-contract residual**.

This section matters because D01 is increasingly surrounded by stronger planner-side world-model papers, and reviewers may be tempted to read better object-level search directly as better deployment-time supervision. Our intended boundary is narrower and more honest: COMET helps explain *why a released packet may be better proposed*, but not automatically *why it remains honest under delayed downstream consume*. By placing object-centric planning into the same subtraction discipline as memory, contact semantics, and refresh density, D01 keeps proposal quality and packet-contract quality explicitly separate.

### 4.3.6 WEAVER-Style Test-Time Planner Support versus Packet-Contract Residual

Recent local evidence from WEAVER [REF: 2606.13672] brings a sharper version of a question that D01 has been tracking for some time: does a world model that becomes substantially more useful at **test-time planning** also become more useful as a **deployment-time supervisor**? WEAVER trains a multi-view world model that simultaneously predicts future latent states and reward, integrates evaluation, policy improvement, and test-time planning under one flow-matching objective, and demonstrates a 38% real-robot success improvement over π0.5 with test-time planning 5–10× faster than prior world-model-based planners. These numbers matter, but they speak primarily to the **planner side** of the world-model stack: better candidate ranking, faster test-time rollouts, and stronger correlation with true success. They do not, on their own, certify that the same world model produces packets that remain honest when consumed by a downstream aerial-manipulation controller under delayed, contact-rich, and drift-burdened execution.

We therefore introduce a **test-time planner support (TTPS)** family and a corresponding subtraction tuple

\[
\Upsilon_t^{ttp} = (\text{PMR},\; \text{TPL},\; \text{PGS},\; \text{BHR}^{ttp},\; \text{CHR}^{ttp}),
\]

where `PMR` measures planner-match relevance (does the world model pick candidate rollouts that the same-cycle planner would also pick), `TPL` measures test-time planning lift (combined success × latency improvement), `PGS` measures same-cycle proposal gain (a packet released under TTPS is preferred at release time), `BHR^{ttp}` measures bind-honest residual after subtracting test-time planner support, and `CHR^{ttp}` measures consume-honest residual after the same subtraction. The reporting rule is conservative and deliberately mirrors the object-centric rule introduced in §4.3.5. If WEAVER-style gains mostly improve `PMR` or `TPL`, the result is frozen as **test-time planner support**. If `PGS` and `BHR^{ttp}` are both positive but `CHR^{ttp}` collapses under delayed consume, the result is frozen at **same-cycle bind-stable planner support**. Only when `CHR^{ttp}` remains positive in the `W2/B1` and `W2/B2` consume-time stress cells, and `CTTPR` does not regress, may the row be promoted to **test-time-planner-subtracted packet-contract residual**.

This section matters for D01 because the WEAVER-style result is exactly the kind of evidence that is easy to over-claim as packet-contract progress. A world model that wins at test-time planning is, by construction, releasing **planner-favored** packets; D01's central question is whether those same packets survive the much harsher journey from `W0` release to `W2` consume under contact and drift. By treating TTPS as a separate family and forcing a `CHR^{ttp}` residual reading, D01 prevents the natural reviewer intuition “WEAVER-like test-time planning works, therefore D01's supervisor works” from leaking into the central claim. WEAVER helps explain *why a released packet may be better preferred at release time*, but not automatically *why it remains honest under delayed downstream consume*.

### 4.3.7 RepWAM-Style Shared Visual-Action Tokenizer as a Representation-Alignment Support Family

A second addition to the local D01 archive comes from RepWAM [REF: 2606.13674], which attacks the world action model (WAM) bottleneck one layer below the planner: instead of building a better rollout generator, RepWAM builds a **shared semantic visual–action tokenizer** that aligns visual tokens to a vision-foundation-model representation and folds the latent action into the same semantic space, then trains the world action model on top of that aligned tokenizer. On RoboTwin 2.0 this reaches Easy 89.3 / Hard 88.4, materially above reconstruction-oriented WAM baselines. From a packet-contract perspective, RepWAM's gains are real and important, but they live on the **representation side** of the world-model stack. A semantically aligned visual–action latent can plausibly improve candidate ranking, latent-action abstraction, and few-shot grounding, but it is, by construction, one step removed from the deployment-time question of whether a released packet remains honest when consumed by an aerial-manipulation controller.

We therefore introduce a **representation-alignment support (RAS)** family and a corresponding subtraction tuple

\[
\Phi_t^{ras} = (\text{VAA},\; \text{AAS},\; \text{AGP},\; \text{BHR}^{ras},\; \text{CHR}^{ras}),
\]

where `VAA` measures visual–action alignment quality in the tokenizer latent space, `AAS` measures abstraction quality of the latent action (how cleanly the latent action separates, conditions, and composes), `AGP` measures address-grounding preservation (does the latent action still refer to the same object / waypoint anchor / clause after alignment), `BHR^{ras}` measures bind-honest residual after subtracting representation-alignment support, and `CHR^{ras}` measures consume-honest residual after the same subtraction. The reporting rule is again conservative. If RepWAM-style gains mostly improve `VAA` and `AAS` but `AGP` drifts across refresh, the result is frozen as **tokenizer-side representation support**. If `AGP` remains stable through bind but `CHR^{ras}` collapses under delayed consume, the result is frozen as **address-stable representation bind support**. Only when `AGP` is stable across emit/refresh/bind/consume and `CHR^{ras}` is positive in the consume-time stress cells may the row be promoted to **representation-subtracted packet-contract residual**.

This matters because RepWAM is the kind of paper a reviewer would naturally cite as evidence that "the world model stack has improved, therefore D01's supervisor has improved." D01's intended boundary is the opposite: RepWAM helps explain *why a released packet may be more cleanly latent-grounded*, but it does not, on its own, certify *why it remains honest under delayed downstream consume*. By placing RepWAM into the same subtraction discipline as WEAVER, COMET, memory, and contact semantics, D01 keeps representation quality and packet-contract quality explicitly separate, and protects the central narrative from a category of upstream-architecture gains that, however real, are not the same thing as delayed-consume packet honesty.
...
### 4.3.8 World-Pilot-Style VLA-WAM Dual-Path Steering as an Architecture-Prior Support Family

The most recent addition to the local D01 archive, World Pilot [REF: 2606.12403], brings yet another subtle but important evidence type into the matched-subtraction audit. World Pilot attacks a problem one step further upstream than RepWAM: rather than rebuilding the world action model's representation or planning layer, it keeps a vanilla VLA backbone and injects World-Action-Model (WAM) priors through **two complementary steering paths** — a latent steering path that conditions the perception stream on scene-evolution latents, and an action steering path that conditions the action generator on the WAM-predicted motion trajectory. The empirical result is striking: zero-shot OOD Total success rate 84.7% on LIBERO-Plus, with consistent gains over viewpoint / geometry / deformable state / pose shift axes on four real robots, while keeping the VLA backbone itself unchanged. The point of absorbing World Pilot here is therefore not to celebrate yet another SOTA number, but to ask whether a paper whose entire contribution lives on the **steering interface** between a frozen VLA and a frozen WAM can be cleanly read as packet-contract progress in D01's sense.

Concretely, World Pilot teaches three things about the steering side of a world-model stack. First, even when neither the VLA nor the WAM is retrained, the dual-path conditioning improves OOD generalization — which means the gain is genuinely about the interface, not about either backbone. Second, the latent steering path works even when the WAM is video-pretrained without any action post-training, which means the world model is contributing a scene-evolution prior rather than a tightly bound action prior. Third, both paths add latency overhead, which means the steering interface must be explicitly evaluated under delayed-consumption rather than same-cycle. All three observations speak to the same point from three different angles: a stronger steering interface is, by construction, one more layer of upstream architecture that can — but does not automatically — produce packets that survive delayed consume.

We therefore introduce an **architecture-prior steering (APS)** family and a corresponding subtraction tuple

\[
\Theta_t^{aps} = (\text{LSQ},\; \text{ASQ},\; \text{SPN},\; \text{APC},\; \text{BHR}^{aps},\; \text{CHR}^{aps}),
\]

where `LSQ` measures latent steering quality (how cleanly scene-evolution latents improve VLA perception in OOD regions), `ASQ` measures action steering quality (how cleanly the WAM-predicted motion prior improves VLA action generation), `SPN` measures dual-path synergy (whether the two paths jointly contribute more than the sum of their parts), `APC` measures architecture-prior consistency across `emit → refresh → bind → consume` (does the steering signal still point at the same object / clause / stage after a refresh), `BHR^{aps}` measures bind-honest residual after subtracting architecture-prior steering support, and `CHR^{aps}` measures consume-honest residual after the same subtraction.

The reporting rule is again deliberately conservative. If World Pilot–style gains mostly improve `LSQ` and `ASQ` under same-cycle evaluation, the row is frozen as **dual-path steering support**. If `SPN` is positive and `APC` is stable through bind but `CHR^{aps}` collapses under delayed consume, the row is frozen at **synergy-stable steering bind support**. If `APC` itself drifts across refresh, the row is frozen at **steering-drift representation support** even when the same-cycle success number looks good. Only when `APC` is stable across `emit → refresh → bind → consume` and `CHR^{aps}` is positive in the consume-time stress cells may the row be promoted to **architecture-prior-steering-subtracted packet-contract residual**. Importantly, we treat World Pilot's zero-shot OOD improvement as a *steering* gain, not a *contract* gain, until the consume-time residual passes the same scrutiny we have already applied to COMET, WEAVER, and RepWAM.

This section matters because World Pilot is the paper that most directly challenges D01's central narrative from the steering side. A reviewer who reads “World Pilot +84.7% Total success rate on LIBERO-Plus, zero-shot OOD, dual-path WAM priors, real-robot wins” will naturally ask whether the world-model stack has improved enough that D01's deployment-time supervisor is now redundant. Our intended answer is the opposite: World Pilot helps explain *why a released packet can be steered into a better same-cycle action*, but it does not, on its own, certify *why the steered packet still points at the same clause / anchor / stage after refresh, and still passes through delayed-consumption bind and consume without silently switching referents*. By placing World Pilot into the same subtraction discipline as COMET, WEAVER, and RepWAM, D01 keeps steering-interface quality and packet-contract quality explicitly separate. This is the same protection we have been building layer by layer in §4.3.5–§4.3.7, and §4.3.8 closes the steering-side leg of that protection.

A practical consequence of this section is that any future VLA + WAM experiment in D01's evaluation bundle must log at minimum `(LSQ, ASQ, SPN, APC, BHR^{aps}, CHR^{aps})` per row, run at least the same three stress cells we use elsewhere (`W1/B1, W2/B1, W2/B2`), and explicitly report whether the steered packet's `APC` survives a refresh-then-bind-then-consume cycle. Without these, a steering-style result must freeze at the strongest `LSQ/ASQ/SPN`-justified support family and is not allowed to inherit any packet-contract language, even if its zero-shot OOD number is the highest in the table.
...
[17] WEAVER: Better, Faster, Longer — An Effective World Model for Robotic Manipulation. arXiv:2606.13672, 2026.
[18] RepWAM: World Action Modeling with Representation Visual-Action Tokenizers. arXiv:2606.13674, 2026.
[19] COMET: Causal Object-Centric MCTS for World-Model Planning. arXiv:2606.14418, 2026.
[20] Looped World Models: Iterative Latent Depth as a Third Scaling Axis. arXiv:2606.18208, 2026.
[21] FR3D: Future Dynamic 3D Reconstruction as a 3D World Model with Disentangled Ego-Motion. arXiv:2606.18250, 2026.

[22] Mem-World: Memory-Augmented Action-Conditioned World Models for Persistent Robot Manipulation. arXiv:2606.18960, 2026.
[23] WAM-RL: World-Action Model Reinforcement Learning with Reconstruction Rewards and Online Video SFT. arXiv:2606.17906, 2026 (CVPR 2026 Workshop).
[TODO: 补充完整引用列表]
