...
| 日期 | 响应的批注 | 执行动作 |
|------|-----------|---------|
| 2026-06-17 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04：严格先做本地优先，复核 **GAM / WorldFly / FLIGHT / WildRoadBench** 四个近30天本地锚点、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment aerial manipulation geometry prior generalization --no-rerank`（结果仍主要回流既有研究状态与少量空中操作锚点，未触发外部扩搜）；在 `PAPER.md` 实质新增 **2.22 Geometry-Predictive Action Models** 与 **3.6 Geometry-Predictive Backbone Support before Embodiment Promotion**，把 GAM 形式化为 `GPS / FGS / ACS / HRS^{geo}` 的几何预测支持族，并同步扩写 **4.1 Setup / 4.2 Main Results / 4.3 Ablation Study**，将 GAM 纳入 matched control families、主表字段与 `A_{-gam}` 审计，明确几何预测 backbone 只能先冻结为 geometry-predictive support，不能直接冒领 shared latent/interface gain |
| 2026-06-17 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04：严格先做本地优先，复核 **GAM / ROVE / WorldFly / FLIGHT / WildRoadBench** 五个近30天本地锚点、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment geometric action model recovery experience selection --no-rerank`（结果仍主要回流 D04 既有方向材料，未触发外部扩搜）；在 `PAPER.md` 实质新增 **2.23 Intervention-Value Recovery as Bounded Supervision Support** 与 **3.7 Intervention-Value Recovery Support before Specialist Promotion**，把 ROVE 形式化为 `RVS / HIS / OVS / HRS^{rove}` 的恢复价值支持族，并同步扩写 **4.1 Setup / 4.2 Main Results / 4.3 Ablation Study**，将 ROVE 纳入 matched control families、主表字段与 `A_{-rove}` 审计，明确 critic-filtered human intervention 只能先冻结为 intervention-value recovery support，不能直接冒领 shared latent/interface gain |
| 2026-06-16 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04：严格先做本地优先，复核 **WorldFly / FLIGHT / WildRoadBench** 三个近30天本地锚点、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment UAV embodiment transfer aerial manipulation morphology generalization --no-rerank`（结果仍主要回流概念页与既有方向材料，未触发外部扩搜）；在 `PAPER.md` 实质补强 **4.3 Ablation Study**，新增 `\mathcal{S}_{row}^{air}=(Q^{wm}_{air},Q^{pilot}_{air},Q^{ground}_{air},HRS^{air-struct},CW,PC)` 与对应 residual 审计，把 **WorldFly** 冻结为 aerial future-world imagination support、**FLIGHT** 冻结为 pilot-reasoning support、**WildRoadBench** 冻结为 grounding-coverage support，明确这些 aerial structuring uplift 必须先被 matched subtraction 消耗后，D04 才允许保留更强的 shared-latent / executability / specialist 叙事；同步更新 PAPER 头部时间戳 |
| 2026-06-15 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04：复核本地近30天锚点 **EmbodiSteer / Self-Other Distinction / Flow-Reversal Steering**、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment generalization self-model retargeting proprioceptive visual correspondence --no-rerank`（结果仍主要回流本地方向材料，未触发外部扩搜）；在 `PAPER.md` 实质补强 **3.4 In-Context Embodiment Residual Modeling**，新增 `\mathbf{r}^{self}_t / \mathbf{r}^{steer}_t / \omega_t / \omega_t^{self} / \omega_t^{steer}` 与 corresponding residual fusion / weakest-honest escalation equations，把 **self-model distinction** 冻结为 shared-space grounding support、把 **flow-reversal steering** 冻结为 action-entry steering support，明确它们都必须先通过 geometry / latent / joint-space gate 后才允许升级为 remaining embodiment structure；同步更新 PAPER 头部时间戳 |
| 2026-06-15 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04：复核本地近30天锚点 **EmbodiSteer / Flow-Reversal Steering / Self-Other Distinction**、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment generalization morphology invariant action representation policy transfer --no-rerank`（结果仍主要回流本地方向材料，未触发外部扩搜）；在 `PAPER.md` 实质补强 **4.2 Main Results**，把 **self-model distinction / occupancy grounding** 明确加入 reviewer-facing 主表 `\Upsilon_{row}^{D04}` 与 canonical rows，新增 **Row I (Self-other distinction control)**，并写死“自我表征带来的 shared-space grounding 增益只能先冻结为 self-model support，不能直接冒领 shared latent/interface gain”的晋升规则；同步更新 PAPER 头部时间戳 |
| 2026-06-15 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04 的 Method 3.x：复核本地近30天锚点 **WorldFly / FLIGHT / WildRoadBench / EmbodiSteer**、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross-embodiment UAV embodiment transfer aerial manipulation morphology generalization --no-rerank`（结果仍主要回流本地概念页与既有方向材料，未触发外部扩搜）；在 `PAPER.md` 实质新增 **3.6 Scene-Graph, Context-Encoding, and Aerial Structuring Supports after the Executability Gate**，把 `SGS / CES / WPS / GGS / HRS^{struct}` 形式化为 executability 之后、latent/specialist 晋升之前的 planner-side world-structuring subtraction gate，明确 aerial scene-graph / context-encoder / future-world / grounding gain 只能先冻结为 support family；同步更新 PAPER 头部时间戳 |
| 2026-06-14 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04 的 Method/Experiments：复核本地已入库 **EmbodiSteer (2606.12965)**、D04 README / PAPER / OWNER_NOTES，并补跑 QMD `cross embodiment geometry latent retargeting joint-space guidance executability EmbodiSteer --no-rerank`（命中主要回流本地方向材料，未触发外部扩搜）；在 `PAPER.md` 实质补强 **4.2 Main Results**，把 `JGS / CAS / LLS / HRS^{js}` 正式并入 reviewer-facing 主表 `\Upsilon_{row}^{D04}`，新增 **EmbodiSteer control / Row G**，并写死“joint-space executable steering 只能先记为 deployment-time executability support”的主结果晋升规则；同步更新 PAPER 头部时间戳 |
| 2026-06-13 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04 的 Method/Experiments：基于本地近30天锚点 **EmbodiSteer / Human2Humanoid / Any2Any / WorldFly / FLIGHT / WildRoadBench** 与 QMD 复核，在 `PAPER.md` 中补强 **3.11 Joint-Space Guidance as a Post-Latent Executability Guard**，修正 `\tilde a_t^\tau / \bar a_t^\tau / \mathcal{E}_{js}` 的可执行公式记号，并在 **4.3 Ablation Study** 新增 `A_{-js}` 与 `\mathcal{A}^{D04,+js}`，把 joint-space guidance / collision-aware steering / limit-aware stabilization 明确冻结为 **deployment-time executability support**，避免零样本部署增益被误升格为 shared latent sufficiency；同步更新 PAPER 头部时间戳 |
| 2026-06-12 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04 的 Method 3.x：基于本地近30天锚点 **WorldFly / FLIGHT / WildRoadBench** 与 QMD 复核，在 `PAPER.md` 重写 **3.10 Executable Geometry-to-Latent Promotion Algorithm**，补出 `\gamma_t^{lat}` / `\gamma_t^{ctx}` 两级激活门、`\mathcal{L}_{total}` 分阶段训练目标、以及 `A_t(c)` 与 `PC_t` 驱动的 six-step matched-control promotion audit；把 geometry-first → latent warm-start → context residual → claim freeze 的执行顺序压成可复现实验协议，并同步更新 PAPER 头部时间戳 |
| 2026-06-10 | [2026-05-27] 推进 Method 3.x 具体技术描述 | 本轮继续主推进 D04 的 Method/Experiments：基于本地新入库 **FLIGHT / WorldFly / WildRoadBench** 与 QMD 复核，在 `PAPER.md` 新增 **3.15 Aerial World-Prediction, Pilot-Reasoning, and Grounding Supports before Embodiment Promotion**，并重写 **4.1–4.3** 为 6 类 control family 的 submission-ready 审计模板；把 WorldFly / FLIGHT / WildRoadBench 明确冻结为 `WPS / PRS^{air} / GGS`，避免空中侧能力被误写成 shared latent sufficiency；同步更新 PAPER 头部时间戳 |
