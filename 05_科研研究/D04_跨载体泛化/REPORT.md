...
## Round 2026-05-24 09:37 CST | 方向: D04_跨载体泛化
- **D04_轻量扫描**：本轮按轮换主推进 **D04 跨载体泛化**，避免与上一轮主推进 **D06** 连续重复，也继续满足近三轮覆盖 `D01/D04/D06` 至少一个。严格先做本地优先：复核 `README.md / PAPER.md / OWNER_NOTES.md / REPORT.md / 研究状态追踪`，确认 `OWNER_NOTES.md` 仍无未响应批注；回扫近 30 天本地 L1 命中 **RIO / HandCDO / D-CLING / GA3T / Human-Robot Copilot / RotVLA**，并补跑 QMD `cross-embodiment representation exposure consumable transfer packet readiness delayed uptake RIO HandCDO D-CLING GA3T --no-rerank`。QMD 结果仍主要回流 **概念页 / D04 README / DexFormer**，没有形成需要正式完整入库的新高价值论文，因此本轮 **高价值扫描命中 4 篇核心本地锚点（RIO、HandCDO、D-CLING、GA3T）**、**正式新增入库 0 篇**，不触发 arXiv / Tavily 外扩。
- **D04_PAPER.md 推进**：本轮直接补强 `PAPER.md` 的 **Experiments**，细化并压实 **4.2.47 Family-Matched Exposure and Bounded-Adaptation Serialization under the Current Local Anchor Set** 与 **4.2.48 Current Evidence-Consistent Freeze after RIO / HandCDO / D-CLING / GA3T Joint Re-read**。核心新增是把当前最容易被混写的四类上游增益正式串成一条 family-matched serialization：**GA3T = terrain-state completion**、**RIO = packet hygiene / representation-infrastructure exposure**、**HandCDO = contact-capable descriptor readability**、**D-CLING = prior-preserving bounded adaptation**；它们都必须先在自己的最弱诚实解释位完成扣账，只有穿过同一 latest honest consumption window 后仍残留的收益，才允许升级成 latent/policy-interface transfer 或 remaining embodiment structure。
- **核心价值**：这一步把 D04 从“会做 family-matched residual routing”继续推进到“连 terrain / infrastructure / descriptor / bounded-adaptation 四条近邻路线也必须先各自停在最弱诚实解释位”。也就是说，D04 现在更明确地区分了 **看得更全、包搬得更稳、接触结构更可读、先验保得更好**，这些都还不等于共享控制接口已经充分。
- **下轮建议**：按轮换优先切 **D01_世界模型** 或 **D07_Isaac强化学习机械臂控制**；若继续回到 D04，下一步应把 `terrain-state completion / representation-infrastructure exposure / contact-capable descriptor exposure / prior-preserving bounded adaptation` 这四条 family 进一步压进唯一主表 schema，并补出与 latest honest consumption window 对齐的统一字段名。

## Round 2026-05-16 09:27 CST | 方向: D04_跨载体泛化
- **D04_轻量扫描**：本轮按轮换切到 **D04 跨载体泛化**，满足近三轮覆盖 D01/D04/D06 至少一个，且避免与上一轮主推进 **D01** 连续重复。严格先做本地优先：复核 `README.md / PAPER.md / REPORT.md`，并回扫本地近期 D04 锚点 **HandCDO / FlexiTac / OmniRobotHome / RIO / PokeVLA / Breaking Lock-In / VistaBot**；本轮尝试补跑近30天 L1 与 QMD，但当前 cron 会话仍受 allowlist 限制，未取得稳定新命中，因此保持 **不扩 arXiv / Tavily**。综合判断为 **高价值新增论文 0 篇（本地锚点复核为主）**、**正式入库 0 篇**。
- **D04_PAPER.md 推进**：本轮直接补强 `PAPER.md` 的 **Experiments**，新增 **4.2.43 Contact-Capable Descriptor Exposure versus Consumable Contact Transfer** 与 **4.2.44 Current Evidence-Consistent Freeze after Descriptor/Exposure Re-read**。核心推进是把最近反复出现的 `descriptor / representation / infrastructure` 三类上游增益再压实一层：即使更丰富的 contact-capable descriptor 让 packet 更可读、接触结构更显性，也不能直接冒领为 latent-transfer sufficiency 或 embodiment transfer，除非这类增益在 **contact / coordination / delayed-uptake** 边界后仍被目标载体真实消费。
- **核心价值**：这一步把 D04 从“会区分 exposure 与 utilization”继续推进到“连 **descriptor exposure** 也要和 **consumable contact transfer** 拆开”。也就是说，D04 现在更明确地区分了 **让接触结构更可读**、**让接触状态更可观测**、以及 **让目标载体真正消费这些结构** 这三层。
- **下轮建议**：按轮换优先切 **D06_空中视觉语言导航** 或 **D07_Isaac强化学习机械臂控制**；若继续回 D04，优先统一 `CEP_exp / CEP_use` 与 `RIG / CW` 记账口径，并把 descriptor-exposure 约束压进唯一主表 schema。

## Round 2026-05-16 03:27 CST | 方向: D04_跨载体泛化
- **D04_轻量扫描**：本轮按轮换继续覆盖 **D04 跨载体泛化**，满足近三轮覆盖 D01/D04/D06 至少一个，且避免与上一轮主推进 **D01** 连续重复。严格先做本地优先：复核 `README.md / PAPER.md / OWNER_NOTES.md / REPORT.md`，回扫本地近 30 天 D04 锚点 **RIO / Breaking Lock-In / PokeVLA / VistaBot / FlexiTac / OmniRobotHome / HandCDO / GA3T**，并补跑 QMD `cross-embodiment representation exposure utilization RIO robotic interaction objects --no-rerank`；结果仍主要回流 **D04 README / PMI 框架综述 / 方向 overview**，没有形成需要正式完整入库的新高价值论文，因此本轮 **高价值新增论文 1 篇本地锚点（RIO，已存在本地笔记）**、**正式入库 0 篇**，未触发 arXiv / Tavily 外扩。
- **D04_PAPER.md 推进**：本轮直接补强 `PAPER.md` 的 **Experiments**，新增 **4.2.39 Representation-Scoped Infrastructure Exposure before Consumable Transfer** 与 **4.2.40 Current Evidence-Consistent Freeze after Local RIO Re-read (Representation + Infrastructure)**。核心新增是把之前偏“representation exposure”的 RIO 读法再压实一层：若 object-centric packet factoring 与统一机器人栈主要带来的是 **packet 更干净 / I/O schema 更一致 / deployment trace 更稳定 / pre-contact readiness 更早显现**，但晚窗 `contact / coordination / delayed uptake` 并未按比例前移，则第一诚实结论只能冻结为 **representation-scoped infrastructure exposure**，不能升级成 latent-interface sufficiency 或 embodiment transfer。
- **核心价值**：这一步把 D04 从“会区分 representation exposure 与 utilization”继续推进到“连 **representation cleanup + infrastructure unification** 一起变好，也不能直接冒领为 transfer 变强”。也就是说，D04 现在更明确地区分了 **packet 准备得更干净**、**packet 搬运得更稳**、以及 **packet 真正被目标载体消费** 这三层。
- **下轮建议**：按轮换优先切 **D06_空中视觉语言导航** 或 **D07_Isaac强化学习机械臂控制**；若继续回到 D04，优先统一 `4.2.37~4.2.40` 的 RIO 相关术语，并把 `Γ_RI` 压进唯一主表 schema。
