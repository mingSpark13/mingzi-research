...
(30) Current packet-centric aerial VLN papers still provide weak evidence on whether packet fields are **actually serialized into and consumed by an execution backend**; without explicit schema alignment to platform logs and controller-facing interfaces, a Semantic Waypoint Packet may remain only a paper-level abstraction rather than a deployable planner-controller contract.
(31) Even when a packet schema is written down, existing aerial VLN work still rarely specifies a **field-by-field mapping into the concrete executor API and logging surfaces**; without an explicit planner-view / verifier-view / executor-view split, failure can be misattributed to semantics, gating, or control consumption.
(32) Current methods also provide weak evidence on whether the *same packet instance* can be traced consistently from planner emission to executor bind time and onward to actual consume time; without this trace, packet-level success can be overstated by silently evaluating different intermediate representations.
(33) Closely related aerial systems still do not define a compact **serialization contract metric** for measuring packet-to-episode-schema completeness, leaving it hard to compare whether different planner outputs are equally executable on a shared backend such as AirSpark WaypointExecutor.
(34) Current methods also provide little evidence on whether a packet that remains semantically plausible after refresh is still **WaypointExecutor-consumable** under real platform constraints such as action serialization, guarded-prefix slicing, and per-step trace consistency across `manifest.json`, `frames.jsonl`, and `actions.jsonl`.
(35) Existing aerial VLN work still offers weak evidence on whether **POI-level final-meters arrival** can be separated from coarse long-horizon success in a way that remains compatible with packet-centered interfaces; many systems can approach the right region, but they do not explicitly show whether semantic packets remain precise enough to preserve sign-level or entrance-level commitment in the last meters before consume time.
(36) Current papers also rarely specify how **POI-grounded reasoning** should interact with packet refresh, verifier checks, and executor binding, making it easy to confuse better late-stage target snapping with genuine consume-time packet accountability.
(37) World-action planning systems such as WorldVLN can already improve short-horizon rollout consistency and waypoint consequence prediction under aerial closed-loop execution, so packet-centric methods must still prove residual value beyond **autoregressive world-action planning support** after matched subtraction.
(38) Existing aerial VLN papers still rarely specify whether planner-side world-action gains survive the executor-view reduction, i.e., whether improvements remain once planner-only latent state is collapsed into the concrete control-facing fields actually consumed by AirSpark WaypointExecutor.
...
### 3.4 WaypointExecutor-Aligned Packet Serialization and Consume-Time Logging Contract
...
To make the Semantic Waypoint Packet more than a paper-side abstraction, we define an explicit alignment contract between the aerial planner and the AirSpark `WaypointExecutor`. The key requirement is that the *same packet instance* must remain identifiable across three logging surfaces: `manifest.json` for episode-level intent and budget context, `frames.jsonl` for time-indexed observation and progress traces, and `actions.jsonl` for executor-bound action commitments. A packet that cannot be serialized consistently into these surfaces may still help planner-side semantic ranking, but it should not be described as a deployable planner-controller interface.

We denote the planner-emitted packet at step \(t\) as \(\mathcal{W}_t = (g_t, p_t, y_t, a_t, c_t, r_t, b_t, h_t)\), where the fields correspond to `goal_type`, `target_pose`, `yaw_hint`, `altitude_band`, `semantic_confidence`, `progress_score`, `safety_budget`, and `handoff_tag`. We then define a serialization operator
\[
C_{ser}(\mathcal{W}_t) = \big(\phi_m(\mathcal{W}_t),\ \phi_f(\mathcal{W}_t),\ \phi_a(\mathcal{W}_t)\big),
\]
where \(\phi_m\), \(\phi_f\), and \(\phi_a\) map planner fields respectively into `manifest.json`, `frames.jsonl`, and `actions.jsonl`. A row is considered **schema-complete** only if all required fields admit non-null, type-stable, executor-readable mappings across these three surfaces.

Concretely, `goal_type`, `safety_budget`, and episode-level packet id should appear in `manifest.json`; `semantic_confidence`, `progress_score`, packet freshness, and context-anchor traces should be updated in `frames.jsonl`; and `target_pose`, `yaw_hint`, `altitude_band`, guarded prefix length, plus executor bind / consume timestamps should be written into `actions.jsonl`. This split makes it possible to distinguish three failure classes that are currently conflated in much of the literature: **serialization failure** (the packet cannot be written cleanly), **bind failure** (the executor rejects or cannot attach to the written action object), and **consume-time drift** (the packet binds successfully but no longer preserves its semantic contract when actually executed).

We therefore attach a five-stage trace tuple to every packet,
\[
E_t = (t^{emit}, t^{auth}, t^{bind}, t^{consume}, c^{trace}_t),
\]
where \(t^{emit}\) is planner emission time, \(t^{auth}\) is verifier approval time, \(t^{bind}\) is WaypointExecutor bind time, \(t^{consume}\) is actual executor consumption time, and \(c^{trace}_t\) measures whether the packet id and field values remain trace-consistent across the three log surfaces. This lets us ask not only whether a planner found a good waypoint, but whether that waypoint survived *serialization → verification → executor bind → delayed consumption* as the same accountable interface object.

This contract is intentionally minimal and platform-facing. It does not claim that AirSpark logging format is universally optimal; rather, it provides a concrete operationalization of packet accountability so that D06 can be judged on the question its owner notes actually care about: whether **3D semantic frontier planning + Semantic Waypoint Packet** yields an output that the AirSpark execution backend can truly consume, audit, and replay. In later experiments, this section motivates reporting `C_{ser}` completeness, bind-failure rate, consume-time drift rate, and trace-consistency rate alongside success metrics, so that packet-centric gains cannot be overstated by planner-only evaluation.
...
### 4.2.28 Minimal Submission-Ready Main-Table Extension for D06 after World-Action and Final-Meters Matching

A submission-ready D06 main table should therefore expose at least the following matched-subtraction fields before any strong packet claim is allowed: `Q^{reason}_{route}`, `Q^{lang}_{narrow}`, `Q^{geo}_{narrow}`, `Q^{unc}_{plan}`, `Q^{world}_{prior}`, `Q^{comp}_{sem}`, `Q^{wam}_{plan}`, `Q^{poi}_{final}`, `C_{ser}`, `e^{auth}`, `e^{bind}`, `e^{consume}`, and `c^{trace}`. This keeps planner-side reasoning, narrowing priors, active perception, world priors, world-action rollout quality, final-meters POI reasoning, serialization completeness, and executor-side consumption honesty in one auditable table.
...
### 4.2.29 World-Action-Subtracted Promotion Ceiling
